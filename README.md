...
*   **報告導出系統 (Report Export)**：
    提供一鍵導出 JSON 格式健康報告的功能。
    *   **命名規則**：`SceneHealthReport_[場景名稱]_[時間戳記].json`。
    *   **儲存路徑**：自動存儲於專案目錄下的 `temp/scene-health-check/`。
    *   **快速導航**：導出後自動開啟檔案總管並選中檔案，且在 UI 上顯示路徑供快速二次存取。
